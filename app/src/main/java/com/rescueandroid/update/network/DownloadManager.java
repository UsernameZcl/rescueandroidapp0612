 ...
 * All right reserved. 
 */
package com.rescueandroid.update.network;
...
import com.rescueandroid.update.network.po.ApkInfo;
import com.rescueandroid.update.network.po.Const;
import com.rescueandroid.update.network.util.HttpRequestUtil;
import com.rescueandroid.update.network.util.IntentUtil;
	...
	public void checkDownload() {
		...
		new Thread() {
			...
			public void run() {
				...
				} else if (true/* checkTodayUpdate() || */ || isAccord) {// 判断今天是否已自动检查过更新
					// ；如果手动检查更新，直接进入
	...
	private void showNoticeDialog() {
		...
		builder.setPositiveButton("下载", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				DownloadCallback downCallback = new DownloadInstall(mContext, apkPath, apkinfo.getApkVersion(),
						apkinfo.getApkCode());
	...
	private boolean checkTodayUpdate() {
		...
		if ("".equals(checkDate) && "".equals(updateDate)) { // 刚安装的新版本，设置详细信息
			...
			sharedPreference.edit().putString(UpdateShared.CHECK_DATE, dateStr)
					.putString(UpdateShared.UPDATE_DATE, dateStr).putString(UpdateShared.APK_VERSION, versionName)
					.putInt(UpdateShared.APK_VERCODE, verCode).commit();
			return true;
		}
		try {
			// 判断defaultMinUpdateDay天内不检查升级
			if ((new Date().getTime() - sdf.parse(updateDate).getTime()) / 1000 / 3600
					/ 24 < Const.defaultMinUpdateDay) {
				return false;
